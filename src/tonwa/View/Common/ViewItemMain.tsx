...
import { BinData, SheetData, StoreSheet } from "../../Store";
import { ViewBud } from "../Bud";
import { RowCols } from "./LabelBox";
import { ViewOperator } from "./ViewOperator";

export function ViewItemMain({ value, isMy, store }: { value: SheetData & BinData & { rowCount: number; }; isMy: boolean; store: StoreSheet; }) {
    const { entity } = store;
    ...
    if (rowCount) {
        ...
        vNo = <span>{no}</span>;
        vRowCount = details.length === 0 ? vTime :
            <>
                {vTime}
