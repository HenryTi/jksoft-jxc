...
import { BizBud, EntitySheet } from "../../Biz";
import { EditBudInline, OnBudChanged } from "../../View/Bud";
...
import { ControlSheetList } from "../../Control";
// import { DashConsole, FormBudsStore, ValuesBudsEditing } from "../../../Store";

export function PageSearch({ control }: { control: ControlSheetList; }) {
    const modal = useModal();
    const { biz } = useUqApp();
    function onSearchAll() {
        modal.open(<PageSearchAll control={control} />);
    }
...
function SearchSheetLink({ sheet }: { sheet: EntitySheet }) {
    ...
    const { caption } = sheet;
    function onSearch() {
        modal.open(<PageSheetSearch control={undefined} />);
    }
    return <SearchLink caption={caption} iconColor="text-info" textColor="text-primary" onClick={onSearch} />;
}

function PageSearchAll({ control }: { control: ControlSheetList; }) {
    const { uq } = useUqApp();
