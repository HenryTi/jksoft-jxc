...
import { LabelBox, NamedProps, Prop, RowCols } from "app/hooks/tool";
...
async function pickFromQueryBase(
    ...
    function PageFromQuery() {
        ...
        let [list, setList] = useState(null as NamedProps[]);
        let [selectedItems, setSelectedItems] = useState<{ [id: number]: NamedProps; }>({});
        async function onSearch(params: any) {
            let queryResults = await queryStore.query(params);
            let pickedArr = queryResults as NamedProps[];
            setList(pickedArr);
        }
        ...
        function onMultipleClick(item: NamedProps, isSelected: boolean) {
        ...
        function onSingleClick(item: NamedProps) {
            closeModal(item);
        }
        function itemBan(item: NamedProps) {
            const { ban } = item;
            if (ban.value > 0) {
        ...
        function ViewItemAtomContent({ value: picked }: { value: NamedProps }) {
        ...
        function ViewItemAtom({ value: picked }: { value: NamedProps }) {
        ...
        function ViewItemSpec({ value: picked }: { value: NamedProps }) {
        ...
        function ViewItemHead({ value: picked }: { value: NamedProps }) {
        ...
        function ViewItemSheet({ value: picked }: { value: NamedProps }) {
