...
interface PageQueryProps<P, R> extends PageProps {
...
export class PageQueryCacheData {
    start: any;
    items: any[];
...
export function PageQuery<P, R>(props: PageQueryProps<P, R>) {
    const isModal = useContext<boolean>(ModalContext);
    if (isModal === true) {
        return <PageQueryBase {...props} isPopFirst={false} isModal={true} />;
    }
    else {
        return <PageQueryNav {...props} />
    }
    function PageQueryNav(props: PageQueryProps<P, R>) {
        const navAction = useNavigationType();
        return <PageQueryBase {...props} isPopFirst={navAction === 'POP'} isModal={false} />;
    }
}

function PageQueryBase<P, R>(props: PageQueryProps<P, R> & { isPopFirst: boolean; isModal: boolean }) {
    ...
    const callQuery = useCallback(async function callQuery(more: boolean = false) {
        ...
        if (isPopFirst === true) {
            let pageCache = uqApp.pageCache.getCache<PageQueryCacheData>();
        ...
        if (!arrResult) {
            ...
            if (typeof query === 'function') {
                ...
                let ret = await query.query(valuesParam);
                let { $page } = ret as any;
                arrResult = $page;
            }
        ...
        if (isModal === false) {
            ...
            if (pageCache !== undefined) {
                pageCache.data = new PageQueryCacheData(pageStart, newItems);
            }
        }
        setLoading(false);
